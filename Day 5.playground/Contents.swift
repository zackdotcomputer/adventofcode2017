...
func escape(maze mazeIn: [Int], withNegaJumps negaJump: Bool = false) -> Int {
    var maze = mazeIn

    var currentPosition = 0
    var stepCount = 0

    while ((0..<maze.count).contains(currentPosition)) {
        let oldJumpInstruction = maze[currentPosition]

        if (!negaJump) {
            maze[currentPosition] = oldJumpInstruction + 1
        } else {
            maze[currentPosition] = oldJumpInstruction + (oldJumpInstruction >= 3 ? -1 : 1)
        }

        currentPosition += oldJumpInstruction
        stepCount += 1
    }
...
// Answer to part 2
escape(maze: input, withNegaJumps: true)
